...
void FormTable::updateData(const QList<QPair<QString, QWidget*>>& inputWidgets) {
    ...
    queryString += " WHERE id_" + name_id.toLower() + " = :id";
    qDebug() << ":id" << id;
...
void FormTable::updateTableView(const QString& tableName) {
    ...
    for (int i = 0; i < record.count(); ++i) {
        ...
        if (fieldName.startsWith("дата")) {
            ...
            QString queryText;
            if (fieldName == "ФИО_пациента_combox") {
                queryText = "SELECT DISTINCT ФИО_пациента FROM Пациенты";
            } else  if (fieldName == "ФИО_врача_combox") {
                queryText = "SELECT DISTINCT ФИО_врача FROM Врачи";
            }
